...
function local_relatorio_treinamentos_get_report_sql() {
    return <<<'RTSQL'
        SELECT
            ...
            u.id                                            AS userid,
            TRIM(u.username)                                AS bas_usuario,
            TRIM(CONCAT(u.firstname, ' ', u.lastname))  AS bas_nome_funcionario,
            TRIM(u.email)                                   AS bas_email,
            TRIM(u.city)                                    AS bas_cidade,
            TRIM(u.idnumber)                                AS opc_numero_identificacao,
            TRIM(u.institution)                             AS opc_instituicao,
            TRIM(u.department)                              AS opc_departamento,
            TRIM(info_cpf.data)                                   AS dp_cpf,
            TRIM(info_cidade.data)                                AS dp_cidade_colaborador,
            TRIM(info_uf.data)                                    AS dp_uf,
            to_timestamp(NULLIF(NULLIF(info_nascimento.data, ''), '0')::bigint) AS dp_data_nascimento,
            TRIM(info_sexo.data)                                  AS dp_sexo,
            TRIM(info_sid.data)                                   AS dp_sid,
            TRIM(info_tipo.data)                                  AS prof_tipo,
            TRIM(info_tipo_desc.data)                             AS prof_descricao_tipo,
            TRIM(info_empresa.data)                               AS prof_numero_empresa,
            TRIM(info_empresa_nome.data)                          AS prof_nome_empresa,
            TRIM(info_filial.data)                                AS prof_codigo_filial,
            TRIM(info_filial_desc.data)                           AS prof_nome_filial,
            TRIM(info_distrito.data)                              AS prof_numero_distrito,
            TRIM(info_distrito_nome.data)                         AS prof_nome_distrito,
            TRIM(info_bandeira.data)                              AS prof_bandeira,
            TRIM(info_local.data)                                 AS prof_numero_local,
            TRIM(info_local_nome.data)                            AS prof_nome_local,
            ...
            TRIM(info_cargo.data)                                 AS prof_codigo_cargo,
            TRIM(info_cargo_nome.data)                            AS prof_cargo,
            TRIM(info_grau.data)                                  AS prof_codigo_grau_instrucao,
            TRIM(info_grau_nome.data)                             AS prof_grau_instrucao,
            TRIM(info_hierarquia.data)                            AS prof_hierarquia,
            TRIM(info_posicao.data)                               AS prof_posicao,
            TRIM(info_diretor.data)                               AS prof_diretor,
            TRIM(info_cargo_diretor.data)                         AS prof_cargo_diretor,
            TRIM(info_diretor_hierarquia.data)                    AS prof_hierarquia_diretor,
            TRIM(info_diretor_posicao.data)                       AS prof_posicao_diretor,
            TRIM(info_gerente_regional.data)                      AS prof_gerente_regional,
            TRIM(info_cargo_regional.data)                        AS prof_cargo_regional,
            TRIM(info_regional_hierarquia.data)                   AS prof_hierarquia_regional,
            TRIM(info_posicao_regional.data)                      AS prof_posicao_regional,
            TRIM(info_gerente_distrital.data)                     AS prof_gerente_distrital,
            TRIM(info_cargo_distrital.data)                       AS prof_cargo_distrital,
            TRIM(info_hierarquia_distrital.data)                  AS prof_hierarquia_distrital,
            TRIM(info_posicao_distrito.data)                      AS prof_posicao_distrital,
            TRIM(info_gestor.data)                                AS gestor,
            TRIM(info_gestor_ai.data)                             AS gestor_ai,
            TRIM(info_gestor_farmaceutico.data)                   AS gestor_farmaceutico,
            TRIM(info_gestor_farmaceutico_ai.data)                AS gestor_farmaceutico_ai,
            TRIM(info_situacao.data)                              AS prof_codigo_situacao,
            TRIM(info_situacao_desc.data)                         AS prof_descricao_situacao,
            ...
            TRIM(info_insc_admissao.data)                         AS insc_data_admissao_inscricao,
            TRIM(info_insc_demissao.data)                         AS insc_data_demissao_inscricao,
